  ...
  if game:get_value("i1029") == 2 and destination == from_outside_house_sick then
    sol.timer.start(1000, function()
      ...
      game:start_dialog("dargor.3.house", game:get_player_name(), function()
        ...
        m:set_target(dest_galen:get_position())
        m:set_speed(32)
        m:start(np2_galen, function()
          ...
          sol.timer.start(2000, function()
            ...
            game:start_dialog("galen.2.house", game:get_player_name(), function()
              -- Move the hero out of the way and Galen to the door concurrently
              hero:get_sprite():set_animation("walking")
              local m2 = sol.movement.create("target")
              m2:set_target(dest_link:get_position())
              m2:set_speed(32)
              ...
              np2_galen:get_sprite():set_animation("walking")
              m3:set_target(from_outside_house_sick:get_position())
              m3:set_speed(48)
              m3:start(np2_galen, function()
                np2_galen:remove()
                hero:unfreeze()
