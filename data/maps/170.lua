...
function sensor_boss:on_activated()
  if game:get_value("i1613") <= 8 then
    if warp:is_enabled() then
      warp:set_enabled(false)
      sol.audio.play_music("boss")
    end
  else map:open_doors("door") end
  if not boss and not warp:is_enabled() then
    ...
    elseif game:get_value("i1613") == 4 and not boss_2 then
    ...
    elseif game:get_value("i1613") == 5 and not boss_2 then
    ...
    elseif game:get_value("i1613") == 8 and not boss_2 then
...
map:register_event("on_update", function(self, destination)
  if not map:has_entities("boss") then
    if game:get_value("i1613") < 8 and not warp:is_enabled() then
      warp:set_enabled(true)
      sol.audio.play_music("underground")
    elseif game:get_value("i1613") >= 8 and not warp:is_enabled() then
      map:open_doors("door")
